...
class Producto(models.Model):
    ...
    verificacion_katy = models.CharField(max_length=20, choices=VERIFICACION_CHOICES, default='no_recibido')
    ...
    def save(self, *args, **kwargs):
        ...
        self.valor_total_unidad = costo + taxes + imp
        self.valor_general = self.valor_total_unidad * Decimal(self.stock)
        ...
        self.ganancia_unidad = self.precio_venta_usd - self.valor_total_unidad
        self.ganancia_total = self.ganancia_unidad * Decimal(self.stock)

        if mercado_soles > 0 and venta_soles > 0:
            self.descuento_soles = mercado_soles - venta_soles
            self.descuento_porcentaje = (self.descuento_soles / mercado_soles) * 100
        else:
